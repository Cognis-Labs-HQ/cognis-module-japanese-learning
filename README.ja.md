...
表示される漢字語彙は漢字表記と原子的なかな発音で終端し、一致する漢字の読みはその表示語彙カードを開くようになりました。置き換えられた非表示の読みレコードはパッケージ内に残しつつ切り離し、既存インストールでも重複した使用元を生じず安全に更新できるようにしています。かなカードと読みカードは、誤解を招く重複バッジや漢字バッジではなく、一般的なかなクラスと発音クラスを使用します。
