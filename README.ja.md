...
このモジュールは、かな・漢字・語彙カード用にローカライズされた **Jisho 辞書** Composer プロバイダーを登録します。まず同梱データセットから完全一致を検索し、作成済みの全フィールドと関係を返します。キャッシュにない場合だけ Jisho を問い合わせ、処理中および完了済みの成功応答は上限付きセッションキャッシュで共有します。外部結果は、レコードやリンクを推測で作らず、正規ラベル、発音、利用可能な JLPT レベル、既存の漢字・完全な読み・原子的かなへのリンクを入力します。
