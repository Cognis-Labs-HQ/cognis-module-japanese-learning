...
原子的および複合的な表記単位は、必須の発音一覧と任意の音声アップロード欄を提供します。外部のプレースホルダー URL は公開せず、Library は認証済みパック資産として同梱された音声だけを受け入れます。専用の助詞レイヤーは文法機能のメタデータを保持し、文レコードは単語と助詞への順序付き参照を維持できます。
...
コンテンツパックには現在使用する正規グラフだけを収録します。各漢字の発音は、その漢字だけを構成するかな読みレコードを参照し、語彙全体の発音は別のレコードとして活用かなを追加できます。たとえば `好` は `す` に解決され、単語 `好き` はその読みに `き` を加えます。

## 直接関係だけを示すかな使用ナビゲーション

かなの「使用元」には、直近の読みレコードだけを表示します。漢字から原子的なかなへ直接リンクせず、`あ` → `あめ` → `雨` の順にたどるため、単に `あ` を含むだけの別の単語や漢字が直接の親として表示されません。
...
文の発音詳細は、現在の `linkRelationships` 契約を使用し、リンク元として `words` と `particles` の両方を宣言します。そのため `がっこうにいく` の読みは、完全な各部分を表示対象の `学校`、助詞 `に`、表示対象の `行く` にリンクし、並行する文読みレコードは作りません。

## 筆順に沿った書き取り練習

すべての仮名カードと漢字カードに、正規化座標、順序付きの単調な時刻サンプル、練習用の許容値を持つ必須の `strokePattern` を追加しました。これは Cognis PR #226 の最新 Study Library 描画契約に対応します。パターンは Unicode 名の KanjiVG パスから曲線を保つサンプリングで直接再生成し、折り返しや輪が表示字形に忠実になるようにしました。出典と CC BY-SA 3.0 の帰属情報はコンテンツマニフェストに保持します。
...
このモジュールは、汎用 `study:library:provider.registerLookupProvider` コントラクトを直接使用して、かな・漢字・語彙カード用にローカライズされた **Jisho 辞書** Composer プロバイダーを登録します。まず同梱データセットから完全一致を検索し、作成済みの全フィールドと関係を返します。キャッシュにない場合だけ Jisho を問い合わせ、処理中および完了済みの成功応答は上限付きセッションキャッシュで共有します。外部結果は、レコードやリンクを推測で作らず、正規ラベル、発音、利用可能な JLPT レベル、既存の漢字・完全な読み・原子的かなへのリンクを入力します。
...
すべての文の発音をホストのタイトル詳細リゾルバーに対して検証し、各かな区間が順序どおりの語彙または助詞レコードを開くようにしました。各漢字は、隣接する語彙ターゲットと完全一致する曖昧でない完全な主要読みを 1 つ公開します。その他の読みはタイトルに使わない別の関係で保持し、逆引き用の全かな依存関係も維持します。
