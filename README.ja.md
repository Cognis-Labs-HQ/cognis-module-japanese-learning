...
表示される漢字語彙は、中間の読み語彙を挟まず、表記を漢字へ、発音を原子的なかなへ直接リンクするようになりました。漢字カードでは、`ねこ` のような語彙的な読みを表示名詞 `猫` へリンクします。表示される語彙項目が読みを表せない場合に限り、非表示の発音カードを残します。かなカードでは、単一のひらがなまたはカタカナバッジと汎用クラス `writing:kana` を使用し、重複タグを避けます。
