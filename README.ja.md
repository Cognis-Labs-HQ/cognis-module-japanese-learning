...
表示語彙には `私`、`水`、`電車`、`先生`、`食べる`、`行く` など一般的な日本語表記を使用し、完全な非表示読みで仮名への経路を維持するようになりました。助詞コレクションは、`学校` の周囲で助詞だけを置き換えるのではなく、人、動物、天気、食事、移動、読書、音楽、時間、仕事にまたがる51件の異なる自然な構文として書き直しました。

## 分類済み・分節リンク付きの読み

すべてのプロバイダー項目に、最新の Cognis Library 契約に従う名前空間付きセマンティッククラスを設定しました。完全な発音は内部構造を保持し、漢字由来の部分は非表示の漢字読み語彙へ、活用かなは原子的なかなへリンクします。そのため `強い` は、リンクのない `つよい` ではなく `つよ` と `い` を通して解決されます。

## レイヤーリンクのガードレール

パックのテストは各スキーマレイヤーのセマンティックロールからリンク要件を導出し、孤立したプロバイダーレコードを拒否します。さらに、リンクを持つ各レイヤーを変更するミューテーションテストにより、切断された学習内容が見逃されないようにしました。
