...
- Do not promote a Kana-primary core card to Kanji unless the complete Kanji, hidden-reading Vocabulary, and atomic-Kana path is intentionally part of the reviewed pack. Prefer a fully linked Kana card over a Kanji label with an unlinked pronunciation.
...
- Follow host-normalized classes for structural roles: definition records use `class: "definition"` and `hidden: true`, ordered sentence records use `class: "composite"`, and particle records use `class: "particle"` with `editable: false`.
- Assign every non-structural provider record a namespaced `class` that describes its semantic role (for example `lexical:noun` or `reading:kanji`); use the host-reserved unnamespaced structural classes above exactly and keep classification distinct from layer placement.
- Sentence pronunciation fields must link through `pronunciation-readings` to one hidden complete-pronunciation Vocabulary record. That record must reconstruct the full reading in order from the closest hidden word-reading Vocabulary and `reading-kana` references for particles or other single non-lexical Kana. Unlinked pronunciation text is allowed only when no authored lower-layer record can represent it, and the exception must be documented and tested.
- Avoid duplicate-labeled parents in a reading card's Used By list. When a single-Kanji visible word and its Kanji would both reference the same Kanji-reading record, insert a hidden complete pronunciation record for the visible word; that wrapper composes from the Kanji reading, so the underlying reading is used by differently labeled Kanji and pronunciation records rather than two identical Kanji labels.
- A complete hidden pronunciation must preserve internal deep links: compose Kanji-derived spans from the closest hidden Kanji-reading Vocabulary and link remaining inflectional Kana through ordered `reading-kana`, whose `presentationRole` is `composition`. Reserve `kana-spelling` for complete alternate spellings reconstructed entirely from Kana. For example, `なおす` composes meaningful `なお` from the `直` reading Vocabulary and its single non-lexical `す` suffix directly from atomic Kana.
- Keep authored composition, spelling, pronunciation, reading, word, and particle links acyclic. Kana characters terminate traversal. A Kanji reading may resolve through one hidden reading Vocabulary record to its ordered Kana, but it must never point back to the Kanji or a lexical record that uses that Kanji.
- A Kana form with its own lexical meaning is ordinary visible Vocabulary, not a hidden `reading:kanji` record. For example, the day-counter `か` is `lexical:counter`; hidden Kanji-reading records exist only to express a Kanji pronunciation and must not inherit a narrower lexical meaning.
