...
- A complete hidden pronunciation must preserve internal deep links: compose Kanji-derived spans from the closest hidden Kanji-reading Vocabulary and link only the remaining inflectional Kana through ordered `kana-spelling`. For example, `つよい` composes `つよ` from the `強` reading Vocabulary and `い` from atomic Kana.
