...
- Assign every provider record a namespaced `class` that describes its semantic role (for example `lexical:noun`, `reading:kanji`, or `syntax:sentence`); keep this classification distinct from layer placement.
