...
すべての非表示発音レコードは、`kana-spelling` を通じて順序付きの原子的なかなから完全なタイトルを直接構成します。読みレコードでは、この関係の表示ロールを `composition` とします。再帰的または誤解を招くカード移動を生むため、読みタイトルを `word-spelling` や `reading-kana` 経由にしてはいけません。
