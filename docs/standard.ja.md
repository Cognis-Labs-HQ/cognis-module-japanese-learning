...
受信パスを置き換えるリリースと同じリリースで、プロバイダーの読みレコードを削除してはいけません。置き換えられたレコードは非表示のまま意味定義を保持し、構造参照を削除して、現在のレコードから参照されないようにします。これにより、ホストに欠落したプロバイダーレコードの整合を強いることなく、反復インポートで既存のデータベース行を更新できます。現在の表示漢字語彙は順序付きの原子的なかなへ直接リンクし、漢字は一致する表示語彙レコードを参照できます。
