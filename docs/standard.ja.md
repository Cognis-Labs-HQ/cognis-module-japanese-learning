...
現在の教材でほかに使用されていないことだけを理由に、有効な助詞を削除してはいけません。パック内の各助詞には、ローカライズ済みの機能メタデータ、順序付き仮名綴り、少なくとも1件の順序付き例文参照が必要です。

## 助詞例文の参照経路

各助詞例文には、漢字を介して構成される語彙を含めます。参照される各漢字は完全な非表示読み語彙レコードへリンクし、その読みは順序付きの原子的な仮名から再構成できなければなりません。

## 定義は内容を説明する

定義には、そのレコードの意味をローカライズして記述します。レコードを例、練習、デモ、プレースホルダーなどの作成上の成果物として説明してはいけません。文レコードには意味のある日本語を収録し、順序付きの関係から表示テキストを再構成できなければなりません。

## プロバイダーグラフの整合性

プロバイダーコンテンツの manifest に `protected: true` を設定します。各文では、順序付きでリンクされた語彙と助詞のラベルを連結した結果が文ラベルに、それらの発音を連結した結果が文の発音にならなければなりません。通常漢字で書く語彙は、漢字構成と完全な非表示読み語彙へリンクし、各読みは順序付きの原子的な仮名へリンクします。

## PR 226 の外部パッケージ契約

manifest とスキーマのメタデータには有限のJSON互換値のみを使用し、ライブラリの往復処理後も変更されないようにします。組み込みフィールド型を優先し、拡張型には宣言的な検証規則を必須とします。複数メディアにはリスト型を使用し、バッジ以外のフィールドを学習者向けフィルターにする場合だけ `detail.filterable: true` を設定します。スキーマ改訂とモジュールのセマンティックバージョンは独立させます。

## 漢字優先の語彙とコーパスの多様性

一般的な漢字表記が存在する表示語彙では、その表記を主ラベルに使用します。全仮名の語彙レコードは、通常仮名で書く語と完全な非表示読みに限定します。大規模な追加内容はコーパス全体として確認し、名詞、動詞、話題、文構造の多様性を必須とします。個々の関係が検証を通っても、1つの基準語の周囲だけを繰り返し置換した内容は採用しません。

## 項目クラスと発音分節

パッケージ内の全レコードに名前空間付きのセマンティック `class` を設定します。完全な非表示発音は、漢字由来の部分を最も近い漢字読み語彙から構成し、残りのかなだけを `kana-spelling` から構成します。順序付き参照は発音を正確に再構成しなければなりません。

## レイヤー構造に基づくリンク検証

各レイヤーのセマンティックロールに従って内容を検証します。漢字の読みは発音フィールドを再現し、表示語彙は表記と発音のリンクを保持し、非表示読み・助詞・文は順序付き参照から表示値を再構成しなければなりません。非表示読み、漢字、助詞、定義を孤立させてはいけません。意図的にリンクを除いたレコードを検査が拒否することもテストで証明します。
