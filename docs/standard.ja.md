...
各レコードを個別にレビューするまでは、より大きなコーパスを追加しません。保持するかな主体の語彙レコードは、順序付き `kana-spelling` 参照からラベルと発音の両方を再構成しなければなりません。完全な漢字と非表示読みの経路もパックに含まれる場合にのみ漢字へ昇格し、発音テキストを作成済みリンクなしで残してはいけません。
