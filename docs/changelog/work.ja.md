...
すべてのプロバイダーレコードに、最新の Cognis PR 226 契約に基づく名前空間付きセマンティッククラスを設定しました。完全な発音は分節単位のナビゲーションを保持します。たとえば `強い` の `つよ` は非表示の `強` 読み語彙を経由し、`い` は原子的なかなを経由します。不規則な複合語や活用の文脈読みでも同じ経路を維持し、契約テストですべてのレコードクラスと再構成された発音を検証します。

- [e531a23](https://github.com/Cognis-Labs-HQ/cognis-module-japanese-learning/commit/e531a23612eaf65b873f0241b0904aed8a3be15c)

## レイヤー構造に基づくリンク保護

スキーマロールに基づく新しいテストで、漢字の読み、語彙の表記と発音、助詞、文、定義が作成済みの経路を保持することを必須にしました。各接続レイヤーから意図的にリンクを除くミューテーションケースは必ず失敗し、孤立検査によって切断された非表示読みやプロバイダーレコードの出荷を防ぎます。監査では、未使用の文法レコード `である` と `行き`、および到達不能な依存レコードも削除しました。
