...
かなバリアントは同じ文字種内に限定し、濁点・半濁点、小書きかな、標準的な拗音の全系列、一般的な促音形を収録します。`し` → `じ` → `じゃ` のように親子チェーンで言語構造を保ち、ひらがなとカタカナは互いに独立させます。
...
文字レイヤーは 1 行 5 枚を指定し、両文字種を標準的な五十音順で並べます。`や` 行、`わ` 行、末尾の `ん` 行で音が存在しない位置を、明示的な空白セルで保持します。`きゃ` や `キュ` などの拡張形はグリッドに置かず、`き` と `キ` の子として展開します。
...
最新の最小グリッド renderer が空白を寸法付きで表示するため、グリッドに明示的な `{ "blank": true }` セルを再導入しました。意図的に音が存在しない位置を詰めず、かな表の空白として表示します。
